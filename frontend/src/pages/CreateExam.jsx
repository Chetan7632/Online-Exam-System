...
import { Plus, Trash2, Sparkles, Send, Check, ChevronRight } from 'lucide-react';
...
const CreateExam = () => {
  ...
  const handlePublishExam = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errData = await response.json();
        throw new Error(errData.message || 'Failed to publish exam');
      }
  ...
  return (
    <div className="dashboard-container">
      <div style={{ marginBottom: '32px' }}>
        <h1 className="title-glow" style={{ fontSize: '2rem', marginBottom: '6px' }}>Exam Architect</h1>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          <div className="glass-panel" style={{ padding: '28px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              {questions.length > 0 && (
                <button onClick={handlePublishExam} className="btn btn-primary" style={{ padding: '8px 16px', fontSize: '0.85rem' }}>
                  Publish Examination
                </button>
              )}
            </div>
                ...
                {questions.map((q, index) => (
                  <div key={q.id} className="glass-card" style={{
                    ...
                    <div style={{ position: 'absolute', right: '16px', top: '16px' }}>
                      <button onClick={() => handleDeleteQuestion(q.id)} className="btn btn-danger" style={{ padding: '8px' }}>
                        <Trash2 size={16} />
                      </button>
                    </div>
