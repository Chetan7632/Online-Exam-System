...
import { Award, ShieldAlert, BookOpen, Clock, Play, ArrowRight, Sparkles, CheckSquare, Brain } from 'lucide-react';
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchPublicExams = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/exams/public');
        if (res.ok) {
  ...
  const sampleExams = [
    ...
    {
      title: "Full-Stack Web Development Proficiency",
      description: "Assess your expertise in React 19, Node.js RESTful API architecture, state management patterns, and database optimizations.",
      duration: 45,
      passingScore: 60,
      questionsCount: 15
    },
    {
      title: "AI Ethics and Safety Standards",
      description: "Verify your knowledge in alignment protocols, reinforcement learning safety boundaries, and systemic compliance audits.",
      duration: 20,
      passingScore: 70,
      questionsCount: 10
    }
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 20px 80px' }}>
      ...
      <section style={{ marginBottom: '60px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            <h2 className="title-glow" style={{ fontSize: '1.8rem', marginBottom: '8px' }}>Exam Catalog</h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>Active chambers open for profile registry evaluation</p>
          </div>
          <Link to="/login" className="btn btn-secondary" style={{ fontSize: '0.85rem', textDecoration: 'none', border: '1px solid rgba(255,255,255,0.05)' }}>
            Access Portal to Begin <ArrowRight size={14} />
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div style={{
            ...
            {displayExams.map((exam, index) => (
              <div key={exam._id || index} className="glass-panel" style={{
                padding: '28px',
                display: 'flex',
                ...
                <div>
                  ...
                  <h3 style={{ fontSize: '1.2rem', color: '#f3f4f6', marginBottom: '12px', fontWeight: '700', lineHeight: '1.4' }}>
                    {exam.title}
                  </h3>
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                    Passing Score: <strong style={{ color: 'var(--text-secondary)' }}>{exam.passingScore}%</strong>
                  </span>
                  <Link to="/login" style={{
                    display: 'flex',
                    alignItems: 'center',
                    ...
                    Begin <Play size={12} fill="#00e5ff" />
                  </Link>
                </div>
              </div>
            ))}
